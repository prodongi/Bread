                 ...
                 float4 Diffuse : TEXCOORD3 ) : COLOR0
{
    ...
    return 1.0f;//float4( tex2D( g_samScene, Tex0 ).xyz, 1.0f ) * I;
}
                       ...
                       out float4 oPos : POSITION )
{
	...
	// 법선벡터 In.n과 광원벡터 L의 내적을 구해서
	// 내적값이 0보다 작다는 것은 V의 방향과 법선의 방향이 90이상이라는 것이다.
	// 이런벡터는 L방향으로 주-욱 늘려주면 된다.
	// 아닌경우는 원래 0을 넣어야 하지만, 0을 넣을 경우 Z-fighting이 일어나기 때문에
	// 그림자메시와 원본메시가 점박이(stipple)출력된다.
	// 이를 막기위해서 0.01을 주었다.
	float dir = dot( vNormal, L );
	if( dir <= 0 )
		value = 10;
	else
		value = 0.01;
	...
	// 정점을 광원진행방향으로 늘려준다.
	float3 P = vPos - (L*value);
	
	// 정점을 클리핑좌표로		
	P = mul( float4( P, 1 ), (float4x3)matWV );
	oPos = mul( float4(P,1), matProj );	
