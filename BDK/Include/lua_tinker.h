...
extern "C"
{
#include "..\\..\\Tool\\LuaTinker_0.5b\\lua\\lua.h"
#include "..\\..\\Tool\\LuaTinker_0.5b\\lua\\lualib.h"
#include "..\\..\\Tool\\LuaTinker_0.5b\\lua\\lauxlib.h"
}
...
template<typename A, typename B>		struct if_<false, A, B> { typedef B type; };
	...
	struct mem_functor
	{
		template<typename RVal>
			...
			template<>
				static int invoke<void>(lua_State *L)  { (read<T*>(L,1)->*upvalue_<void(T::*)(T1,T2,T3,T4,T5)>(L))(read<T1>(L,2),read<T2>(L,3),read<T3>(L,4),read<T4>(L,5),read<T5>(L,6),read<T6>(L,7),read<T7>(L,8)); return 0; }
	};
