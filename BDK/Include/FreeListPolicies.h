...
: public PlacementNewCreationHelper< T >
...
public:
	...
	unsigned int GetNumBlocksAllocated() const
	{
		return SharedFreeList::GetNumBlocksAllocated( sizeof(T) );
	}

	unsigned int GetNumberOfBlocksInUse() const
	{
		return SharedFreeList::GetNumBlocksInUse( sizeof(T) );
	}

	unsigned int GetPeakNumberOfBlocksInUse() const
	{
		return SharedFreeList::GetPeakNumberOfBlocksInUse( sizeof(T) );
	}
...
protected:
	...
	void Push( void* pItem )
	{
		SharedFreeList::Deallocate( pItem, sizeof(T) );
	}

	void* Pop()
	{
		return SharedFreeList::Allocate( sizeof(T) );
	}

	void Grow( unsigned int numBlocks )
	{
		SharedFreeList::Grow( numBlocks, sizeof(T) );
	}

	// This class only works on objects that fit inside one of SharedFreeList's internal lists
	STATIC_CHECK( sizeof(T) <= SharedFreeList::maxBytes, 
